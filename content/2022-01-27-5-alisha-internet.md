...
Теперь ты понял что Windows и MacOS проприетарные операционные системы и запрещают тебе всё что можно запретить, \*BSD имеет пермиссивную лицензию (и разрешает корпорация воровать код), а ОС Linux использует вирусную лицензию GNU GPL и разрешает всё, но ты не можешь изменить авторов и закрыть код. А если ты модифицируешь код (fork'аешь его), то ты становишься автором кода (автором fork'a) и сохраняешь авторство людей которые писали части этого кода до тебя.
