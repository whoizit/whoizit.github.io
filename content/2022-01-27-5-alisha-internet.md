...
Я пользуюсь самыми разными Linux'ами c 2007-2008 года и могу говорить о них часами. 
...
Теперь ты понял что Windows и MacOS проприетарные операционные системы и запрещают тебе всё что можно запретить, \*BSD имеет пермиссивную лицензию (и разрешает корпорация воровать код), а ОС Linux использует вирусную лицензию GNU GPL и разрешает тебе всё, но ты не можешь изменить авторов и закрыть код. А если ты модифицируешь код (fork'аешь его), то ты становишься автором кода (автором fork'a) и сохраняешь авторство людей которые писали части этого кода до тебя.
...
*(s6 это ремейк runit, а runit это ремейк daemontools. И они прекрасны своей простой и логикой работы. После понимания того как работает s6 - systemd покажется страшным переусложнённым монстром)*
